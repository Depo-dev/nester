...
fn setup() -> (Env, Address, Address, Address) {
    ...
    env.as_contract(&cid, || initialize(&env, &admin));
    (env, admin, other, cid)
}
...
fn initialize_does_not_grant_operator_to_admin() {
    let (env, admin, _, cid) = setup();
    assert!(!read(&env, &cid, || has_role(&env, &admin, Role::Operator)));
}
...
fn initialize_twice_panics() {
    let (env, admin, _, cid) = setup();
    invoke(&env, &cid, || initialize(&env, &admin));
}
...
fn require_role_passes_for_authorised_account() {
    ...
    read(&env, &cid, || {
        ...
        require_role(&env, &operator, Role::Operator);
    });
}
...
fn require_role_panics_when_account_lacks_role() {
    ...
    read(&env, &cid, || require_role(&env, &other, Role::Admin));
}
...
fn transfer_admin_two_step_happy_path() {
    ...
    invoke(&env, &cid, || transfer_admin(&env, &admin, &new_admin));
...
fn wrong_address_cannot_accept_admin() {
    ...
    invoke(&env, &cid, || transfer_admin(&env, &admin, &new_admin));
    invoke(&env, &cid, || accept_admin(&env, &imposter));
}
...
fn accept_admin_without_proposal_panics() {
    let (env, _, other, cid) = setup();
    invoke(&env, &cid, || accept_admin(&env, &other));
}
...
fn non_admin_cannot_propose_admin_transfer() {
    let (env, _, other, cid) = setup();
    let target = Address::generate(&env);
    invoke(&env, &cid, || transfer_admin(&env, &other, &target));
}
...
fn admin_count_is_consistent_after_full_transfer() {
    ...
    invoke(&env, &cid, || grant_role(&env, &admin, &third, Role::Admin));
