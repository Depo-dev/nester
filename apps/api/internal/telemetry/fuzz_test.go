...
func FuzzRedactValueNeverLeaksSeed(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, prefix, suffix string) {
		...
		// A long prefix of the seed is also a disclosure, not just the whole.
		if len(fakeStellarSecret) >= 24 && strings.Contains(got, fakeStellarSecret[:24]) {
			t.Fatalf("partial seed leaked with prefix=%q suffix=%q -> %q", prefix, suffix, got)
		}
